...
if not os.path.exists('config.yaml'):
    st.error("Fatal Error: `config.yaml` not found. Please create it.")
    st.stop()
...
def get_openai_key():
    ...
    if not key: st.error("Please add your OpenAI API key in the sidebar!"); st.stop()
    return key

def openai_client(): return openai.OpenAI(api_key=get_openai_key())

def chat_completion(messages, model="gpt-4o", temperature=0.3, max_tokens=1500):
    try: return openai_client().chat.completions.create(model=model, messages=messages, temperature=temperature, max_tokens=max_tokens)
    except Exception as e: st.error(f"OpenAI Error: {e}"); st.stop()

def text_to_speech(text, voice="alloy"):
    try: return openai_client().audio.speech.create(model="tts-1", voice=voice, input=text)
...
def transcribe_audio(audio_bytes):
    try:
        with io.BytesIO(audio_bytes) as file:
            ...
            return openai_client().audio.transcriptions.create(model="whisper-1", file=file, response_format="text")
    except Exception as e: st.warning(f"Whisper transcription failed: {e}"); return None

def extract_text(file):
    if file.name.lower().endswith(".pdf"): text = "".join(page.extract_text() or "" for page in PyPDF2.PdfReader(file).pages)
    elif file.name.lower().endswith(".txt"): text = file.read().decode("utf-8")
    else: return None
...
def generate_questions(resume, role, experience, num_questions, model):
    ...
    except Exception as e: st.error(f"Error generating questions: {e}"); return None
...
def evaluate_answer(question, answer, resume, model):
    ...
    except Exception as e: return {"score": 0, "feedback": f"Evaluation error: {e}", "better_answer": "N/A"}
...
def summarize_session(questions, answers, resume, model):
    ...
    try:
        ...
        json_match = re.search(r'\{.*\}', response.choices[0].message.content, re.DOTALL)
        return json.loads(json_match.group(0)) if json_match else {}
    except Exception as e: st.error(f"Error summarizing session: {e}"); return {}
...
def sidebar():
    ...
    st.sidebar.markdown("---"); st.sidebar.markdown("### Interview Settings")
    ...
    if uploaded_file:
        ...
        if st.button("Start Interview", type="primary"):
            if resume and name and get_openai_key():
                st.session_state.update({"resume": resume, "candidate_name": name, "role": role, "q_count": q_count, "answers": [], "transcript": [], "current_q": 0, "stage": "interview"})
                with st.spinner("Generating personalized questions..."): st.session_state.questions = generate_questions(resume, role, "Mid-Level", q_count, MODELS["GPT-4o"])
                st.rerun()
            else: st.warning("Please ensure all fields are complete and an API key is provided.")

def interview_section():
    idx = st.session_state.current_q; questions = st.session_state.get("questions", [])
    if not questions or idx >= len(questions): st.session_state.stage = "summary"; st.rerun()
    q = questions[idx]

    # --- NEW TWO-COLUMN LAYOUT ---
    col1, col2 = st.columns([2, 1.5]) # Left column is wider

    with col1: # Left Column for actions
        st.subheader(f"Question {idx+1}/{len(questions)}: {q['topic']} ({q['difficulty']})")
        ...
        if st.button("Stop and Submit Answer", type="primary"):
            ...
            with st.spinner("Transcribing and evaluating your answer..."):
                ...
                if answer_text:
                    ...
                    st.session_state.transcript.append(f"**Q{idx+1}:** {q['text']}")
                    st.session_state.transcript.append(f"**A:** {answer_text}\n")
                    st.session_state.current_q += 1
                    st.rerun()
                else: st.error("Transcription failed. Please try recording again.")

    with col2: # Right column for information
        with st.container(border=True):
            st.subheader("AI Feedback on Last Answer")
            if not st.session_state.answers:
                st.info("Your first feedback will appear here after you answer a question.")
            else:
                last_eval = st.session_state.answers[-1]
                st.metric("Score", f"{last_eval.get('score', 'N/A')}/10")
                st.markdown("**Feedback:**")
                st.write(last_eval.get('feedback', 'No feedback available.'))
                st.markdown("**Suggested Answer:**")
                st.write(last_eval.get('better_answer', 'No suggested answer available.'))

        with st.container(border=True):
            st.subheader("Full Interview Transcript")
            if not st.session_state.transcript:
                st.info("The transcript will build here as you answer questions.")
            else:
                for entry in st.session_state.transcript: st.markdown(entry)

def summary_section():
    st.header("Step 3: Interview Summary")
    with st.spinner("Generating final summary..."): summary = summarize_session(st.session_state.questions, st.session_state.answers, st.session_state.resume, MODELS["GPT-4o"])
